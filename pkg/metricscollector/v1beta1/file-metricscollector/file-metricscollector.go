...
func parseLogs(logs []string, metrics []string, filters []string) (*v1beta1.ObservationLog, error) {
	...
	for _, logline := range logs {
		...
		if len(ls) != 2 {
			klog.V(2).Warningf("Metrics will not have timestamp since %s doesn't begin with timestamp string", logline)
		} else {
			if _, err := time.Parse(time.RFC3339Nano, ls[0]); err != nil {
				klog.V(2).Warningf("Metrics will not have timestamp since error parsing time %s: %v", ls[0], err)
			} else {
				timestamp = ls[0]
			}
		}
